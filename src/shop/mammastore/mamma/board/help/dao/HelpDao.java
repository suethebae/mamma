...
public class HelpDao {
	...
	public ArrayList<HelpVo> getHelpList(int mber_sq, Pagenation pagenation) {
		...
		try {
			pstmt = con.prepareStatement(
					"select help_sq, sj, date_format(i1.dttm,'%Y-%m-%d %H:%i') as dttm, i.mber_sq, id, answer_fl from inf_help_tb i1 left join inf_mber_tb i on i1.mber_sq=i.mber_sq where i.mber_sq=? order by dttm desc limit ?,?");
			...
			while (rs.next()) { // 다음줄이 null(false) 될떄까지 반복실행
				...
				helpVo.setDttm(rs.getString("dttm"));
				helpVo.setAnswer_fl(rs.getBoolean("answer_fl"));
	...
	public HelpVo getHelpDetail(int help_sq) {
		...
		try {
			pstmt = con.prepareStatement(
					"select i.help_sq, i1.mber_sq, i1.id, i.answer, i.cntnt, i.sj, date_format(i.dttm,'%Y-%m-%d %H:%i') as dttm, i1.phone, i1.email, i.order_cd, i.answer_fl"
					+ " from inf_help_tb i LEFT JOIN inf_mber_tb i1 on i.mber_sq=i1.mber_sq where i.help_sq=?");
	...
	public ArrayList<HelpVo> getOrderCode(int mber_sq) {
		...
		ArrayList<HelpVo> list = new ArrayList<HelpVo>();
		try {
			pstmt = con.prepareStatement("select order_cd from inf_order_tb WHERE mber_sq=?");
			pstmt.setInt(1, mber_sq);
			rs = pstmt.executeQuery();
			while (rs.next()) { // 다음줄이 null(false) 될떄까지 반복실행
				HelpVo helpVo = new HelpVo();
				helpVo.setOrder_cd(rs.getString("order_cd"));
				list.add(helpVo);
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public int getHelpCount(int mber_sq) {
		...
		int count = 0;
		try {
			pstmt = con.prepareStatement("select count(help_sq) from inf_help_tb where mber_sq=?");
			pstmt.setInt(1, mber_sq);
			rs = pstmt.executeQuery();
			if (rs.next()) {
				count = rs.getInt(1);
