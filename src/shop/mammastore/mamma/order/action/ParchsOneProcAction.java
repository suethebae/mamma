...
public class ParchsOneProcAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		// 데이터 유효성 검사
		if (RegExp.isEmpty(itm_sq) || RegExp.isEmpty(itm_cnt) || RegExp.isEmpty(nm) || RegExp.isEmpty(pc)
				|| RegExp.isEmpty(phone) || RegExp.isEmpty(email) || RegExp.isEmpty(zip_cd) || RegExp.isEmpty(adres)
				|| agree == null) {
			response.setContentType("text/html; charset=UTF-8");
			PrintWriter out = response.getWriter();
			out.println("<script>alert('잘못된 접근입니다.'); loaction.href='/'; </script>");
