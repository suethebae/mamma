	...
	<form action="/order/parchsProc" method="post" id="parchsProc">
		<table>
			...
			<tr>
				...
				<td><img src="<%=list.get(i).getFl_pth()%>" alt="" /></td>
				<td><%=list.get(i).getItm_nm()%></td>
				<td><%=list.get(i).getItm_pc()%></td>
				<td><%=list.get(i).getItm_cnt()%></td>
				<td><%=list.get(i).getItm_cnt() * list.get(i).getItm_pc()%><input type="hidden" name="cart_sq" value="<%=list.get(i).getCart_sq()%>" /></td>
			</tr>
			<%
			...
			%>
			<tr>
				<td colspan="6">총 금액 <%=total%> <input type="hidden" name="pc" value="<%=total%>" readonly="readonly" /></td>
			</tr>
		</table>
		<div>
			<h1>구매자 정보</h1>
