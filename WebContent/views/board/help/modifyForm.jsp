...
<body>
	...
	<section id="myHelp__modi">
		<div class="myHelp__modi">
			<div class="myHelp__modi__main">
				...
				<form action="/help/modifyProc" method="post" id="editorForm">
					...
					<div>
						주문 코드 <select name="order_cd" id="order_sq">
							<option value=""></option>
							<%
							for (int i = 0; i < list.size(); i++) {
							%>
							<option value="<%=list.get(i).getOrder_cd()%>"
								<%=vo.getOrder_cd().equals(list.get(i).getOrder_cd()) ? "selected" : ""%>><%=list.get(i).getOrder_cd()%></option>
							<%
							}
							%>
						</select>
