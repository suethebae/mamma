...
public class AitemDao {
	...
	public int register(AitemVo aitemVo) {
		...
		try {
			pstmt = con
					.prepareStatement("insert into inf_itm_tb (nm,pc,stock,cntnt,fl_pth,ctgry_sq) values(?,?,?,?,?,?)");
	...
	public ArrayList<AitemVo> getItemList() {
		...
		try {
			pstmt = con.prepareStatement(
					"select itm_sq, sttus_fl, nm, pc, dttm, fl_pth, stock, ctgry_sq from inf_itm_tb where del_fl=0");
	...
	public int modify(AitemVo aitemVo) {
		...
		try {
			if (aitemVo.getFl_pth() == null) {
				pstmt = con.prepareStatement(
						"update inf_itm_tb set nm=?, pc=?, stock=?, cntnt=?, ctgry_sq=? where itm_sq=? and del_fl=0");
				pstmt.setString(1, aitemVo.getNm());
				pstmt.setInt(2, aitemVo.getPc());
				pstmt.setInt(3, aitemVo.getStock());
				pstmt.setString(4, aitemVo.getCntnt());
				pstmt.setInt(5, aitemVo.getCtgry_sq());
				pstmt.setInt(6, aitemVo.getItm_sq());
				count = pstmt.executeUpdate();
			} else {
				pstmt = con.prepareStatement(
						"update inf_itm_tb set nm=?, pc=?, stock=?, cntnt=?, fl_pth=?, ctgry_sq=? where itm_sq=? and del_fl=0");
				pstmt.setString(1, aitemVo.getNm());
				pstmt.setInt(2, aitemVo.getPc());
				pstmt.setInt(3, aitemVo.getStock());
				pstmt.setString(4, aitemVo.getCntnt());
				pstmt.setString(5, aitemVo.getFl_pth());
				pstmt.setInt(6, aitemVo.getCtgry_sq());
				pstmt.setInt(7, aitemVo.getItm_sq());
				count = pstmt.executeUpdate();
			}
