...
public class AnoticeDao {
	...
	public ArrayList<AnoticeVo> getNoticeList() {
		...
		try {
			pstmt = con.prepareStatement(
					"select mngr_sq, notice_sq, cntnt, sj, dttm from inf_ntbd_tb where del_fl=0");
	...
	public AnoticeVo getNoticeDetail(int notice_sq) {
		...
		try {
			pstmt = con.prepareStatement("select * from inf_ntbd_tb where notice_sq=? and del_fl=0");
	...
	public int modify(AnoticeVo anoticeVo) {
		...
		try {
			pstmt = con.prepareStatement(
					"update inf_ntbd_tb set sj=?, cntnt=?, Mngr_sq=? where notice_sq=? and del_fl=0");
			pstmt.setString(1, anoticeVo.getSj());
			pstmt.setString(2, anoticeVo.getCntnt());
			pstmt.setInt(3, anoticeVo.getMngr_sq());
			pstmt.setInt(4, anoticeVo.getNotice_sq());
